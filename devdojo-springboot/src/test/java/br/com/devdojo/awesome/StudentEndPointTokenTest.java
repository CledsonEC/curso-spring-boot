...
public class StudentEndPointTokenTest {
	...
	public void configProtectedHeaders() {
		String str = "{\"username\":\"pacheco\", \"password\":\"devdojo\"}";
	...
	public void configAdminHeaders() {
		String str = "{\"username\":\"cledson\", \"password\":\"devdojo\"}";
		HttpHeaders headers = restTemplate.postForEntity("/login", str, String.class).getHeaders();
	...
	public void configWrongHeaders() {
		...
		headers.add("Authorization", "dkfslçdkflsd");
	...
	public void listStudentsWhenTokenIsIncorrectShouldReturnStatusCode403() {
		ResponseEntity<String> response = restTemplate.exchange("/v1/protected/students/", GET, wrongHeader,
				String.class);
	...
	public void listStudentsWhenTokenIsCorrectShouldReturnStatusCode200() {
		ResponseEntity<String> response = restTemplate.exchange("/v1/protected/students/1", GET, protectedHeader,
				String.class);
	...
	public void getStudentsByIdWhenTokenIsCorrectShouldReturnStatusCode200() {
		ResponseEntity<Student> response = restTemplate.exchange("/v1/protected/students/1", GET, protectedHeader,
				Student.class);
		Assertions.assertThat(response.getStatusCodeValue()).isEqualTo(200);
	}
	...
	public void deleteWhenUserHasRoleAdminAndStudentExistsShouldReturnStatusCode200() {
		...
		ResponseEntity<String> exchange = restTemplate.exchange("/v1/admin/students/1", DELETE, adminHeader,
				String.class);
	...
	public void deleteWhenUserHasRoleAdminAndStudentDoesNotExistShouldReturnStatusCode404() throws Exception {
		...
		BDDMockito.doNothing().when(studentRepository).deleteById(9L);
		mockMvc.perform(MockMvcRequestBuilders.delete("/v1/admin/students/{id}", -9L).header("Authorization", token))
	...
	public void createWhenNameIsNullShouldReturnStatusCode400BadRequest() throws Exception {
		...
		ResponseEntity<String> response = restTemplate.exchange("/v1/admin/students/", POST,
	...
	public void createShouldPersistDataAndReturnStatusCode201() throws Exception {
		...
		BDDMockito.when(studentRepository.save(student)).thenReturn(student);
		ResponseEntity<Student> response = restTemplate.exchange("/v1/admin/students/", POST,
